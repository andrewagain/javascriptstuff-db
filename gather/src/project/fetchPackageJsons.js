 ...
 */
function fetchPackageJson(packageJsonUrl, callback) {
 ...
 */
module.exports = function fetchPackageJsons(
  githubRepoPath,
  packageJsonPaths,
  callback
) {
  const packageUrls = packageJsonPaths.map(
    packageJsonPath =>
      `https://raw.githubusercontent.com/${githubRepoPath}/master/${packageJsonPath}`
  );
  async.map(packageUrls, fetchPackageJson, callback);
};
